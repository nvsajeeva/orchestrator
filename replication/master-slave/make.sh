...
if [ "${UP_OR_DOWN}" == "down" ]; then
  ...
  rm -f run_bash_* run_mysql_* run_inspect_* run_logs_* cnf_files/*.bak
...
sed -i .bak "s/report-host=\".*\"/report-host=\"${MASTER_NODE}\"/" cnf_files/my.cnf.master
sed -i .bak "s/report-host=\".*\"/report-host=\"${SLAVE01_NODE}\"/" cnf_files/my.cnf.slave01
sed -i .bak "s/report-host=\".*\"/report-host=\"${SLAVE02_NODE}\"/" cnf_files/my.cnf.slave02
